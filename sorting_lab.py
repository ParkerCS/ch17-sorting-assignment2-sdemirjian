...
for pos in range(1, len(nba_leader_list)):
    key_pos = pos
    scan_pos = key_pos - 1
    key_val = nba_leader_list[key_pos]

    while nba_leader_list[scan_pos][1] > key_val[1] and scan_pos >= 0:
        nba_leader_list[scan_pos + 1] = nba_leader_list[scan_pos]
        scan_pos -= 1

    nba_leader_list[scan_pos + 1] = key_val
